...
def extract_cnn_features(model: CNN_HAR_KS, images: np.ndarray, img_scaler: StandardScaler) -> np.ndarray:
    scaled = img_scaler.transform(images.reshape(
        len(images), -1)).reshape(images.shape).astype(np.float32)
    model.eval()
    feats = []
    with torch.no_grad():
...
def forecast_next_rv(stock_data: pd.DataFrame, cnn_model: CNN_HAR_KS,
    ...
    valid_comp = components.iloc[21:].reset_index(
        drop=True)  # drop burn-in, keep all
    if len(valid_comp) == 0:
    ...
    cnn_feat = extract_cnn_features(
        cnn_model, image[-1:], img_scaler)  # last row only
...
def fit_ridge_for_ticker(ticker: str, cnn_model: CNN_HAR_KS, img_scaler) -> dict:
    ...
    bundle = {"regressor": regressor,
              "rv_scaler": rv_scaler, "feat_scaler": feat_scaler}
    os.makedirs(MODELS_DIR, exist_ok=True)
