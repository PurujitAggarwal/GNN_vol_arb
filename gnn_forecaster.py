...
class GNNForecaster:
    ...
    def get_forecast(self, current_date: pd.Timestamp, horizon: int = 1) -> dict:
        ...
        Returns:
            dict of {ticker: sigma_forecast_annualized}
        """
        self._maybe_refit(current_date, horizon)

        # Build features up to current_date
        rv_slice = self._rv_df.loc[:current_date]
        features = build_har_features(rv_slice)
        V = features["V"]
        tickers = features["tickers"]

        if len(V) == 0:
            if self.verbose:
                print(f"  [!] No valid features for {current_date.date()}")
            return {t: None for t in self.forecast_tickers}

        # Predict using the latest day's features
        V_latest = V[-1:, :, :]  # shape (1, N, 3)
        preds = predict(self._models[horizon],
                        V_latest, self._W)  # shape (1, N)
        preds = preds[0]  # shape (N,)

        # Apply bias correction (learned during refit from validation set)
    ...
    def _refit(self, current_date: pd.Timestamp, horizon: int):
        ...
        scale = np.clip(scale, 0.5, 2.0)  # don't let correction be too extreme
