...
class ImpliedVolSurface:
    ...
    def __init__(self, spot_price, risk_free_rate, dividend_yield=0.0, strikes=None, maturities=None, market_prices=None, verbose=False):
        ...
        self.maturities = np.asarray(
            maturities) if maturities is not None else None
    ...
    def implied_volatility(self, market_price, K, T, option_type='call'):
        ...
        # European IV
        european_iv = self.european_implied_vol(
    ...
    def _compute_iv_cell(self, i, j):
        ...
        try:
            iv = self.implied_volatility(
                price, self.strikes[i], self.maturities[j], 'call')
            return i, j, iv
    ...
    def print_optimal_straddle_greeks(self, optimal, metric):
        ...
        print(
            f"Maturity:      {optimal['maturity']:.3f} years ({optimal['maturity']*12:.1f} months)")
    ...
    def find_optimal_straddle(self, strikes, maturities, iv_surface, metric='cheapest'):
        ...
        for i, K in enumerate(strikes):
            for j, T in enumerate(maturities):
                ...
                vega_theta_ratio = abs(
                    greeks['vega'] / greeks['theta']) if greeks['theta'] != 0 else 0
    ...
    def calculate_straddle_pnl_at_forecast(self, K, T, current_iv, forecast_iv, spot_move=0):
        ...
        if spot_move != 0:
            ...
            spot_move_pnl = greeks['delta'] * spot_change + \
    ...
    def european_implied_vol(self, market_price, K, T, option_type='call'):
        ...
        if option_type == 'call':
            def objective(sigma): return self.black_scholes_call(
                self.spot_price, K, T,
                self.risk_free_rate, sigma,
                self.dividend_yield
            ) - market_price
        else:
            def objective(sigma): return self.black_scholes_put(
                self.spot_price, K, T,
                self.risk_free_rate, sigma,
                self.dividend_yield
            ) - market_price
