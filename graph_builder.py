...
class GraphBuilder:
    ...
    def build(self) -> dict:
        ...
        n_actual = int(A.sum())
        self.sparsity = (n_actual / n_possible) * \
            100 if n_possible > 0 else 0.0

        print(
            f"  Graph: {self.n_stocks} stocks, {n_actual // 2} edges, sparsity {self.sparsity:.1f}%")
...
if __name__ == "__main__":
    ...
    raw = yf.download(ALL_TICKERS, start="2016-01-01",
    ...
    print(
        f"Returns: {log_returns.shape[0]} days x {log_returns.shape[1]} stocks")
