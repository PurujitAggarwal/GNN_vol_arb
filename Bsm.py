...
class DeltaHedger:
    ...
    def calculate_hedge_position(
        ...
        if num_straddles <= 0 or contract_multiplier <= 0:
            raise ValueError(
                "num_straddles and contract_multiplier must be positive")
        ...
        option_delta_shares = position_sign * \
            straddle_delta * contract_multiplier * num_straddles
    ...
    def analyze_rehedge_points(self, K, T, sigma, spot_range=0.10):
        ...
        rehedge_data = []
        initial_delta = self.calculate_straddle_delta(
            self.spot_price, K, T, self.risk_free_rate, sigma, self.dividend_yield)

        for spot in spots:
            current_delta = self.calculate_straddle_delta(
                spot, K, T, self.risk_free_rate, sigma, self.dividend_yield)
    ...
    def calculate_gamma_pnl(self, K, T, sigma, spot_move):
        d1 = (np.log(self.spot_price / K) + (self.risk_free_rate -
              self.dividend_yield + 0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
        gamma = np.exp(-self.dividend_yield * T) * norm.pdf(d1) / \
            (self.spot_price * sigma * np.sqrt(T))
        gamma_pnl = 0.5 * gamma * (spot_move ** 2)
        return gamma_pnl

    def calculate_vega_pnl(self, K, T, current_iv, forecast_iv):
        call_curr = self.black_scholes_call(
            self.spot_price, K, T, self.risk_free_rate, current_iv, self.dividend_yield)
        put_curr = self.black_scholes_put(
            self.spot_price, K, T, self.risk_free_rate, current_iv, self.dividend_yield)
        straddle_curr = call_curr + put_curr

        call_forecast = self.black_scholes_call(
            self.spot_price, K, T, self.risk_free_rate, forecast_iv, self.dividend_yield)
        put_forecast = self.black_scholes_put(
            self.spot_price, K, T, self.risk_free_rate, forecast_iv, self.dividend_yield)
        straddle_forecast = call_forecast + put_forecast

        vega_pnl = straddle_forecast - straddle_curr
        vega_pnl_pct = (vega_pnl / straddle_curr *
                        100) if straddle_curr != 0 else 0

        d1 = (np.log(self.spot_price / K) + (self.risk_free_rate -
              self.dividend_yield + 0.5 * current_iv**2) * T) / (current_iv * np.sqrt(T))
        vega = self.spot_price * \
            np.exp(-self.dividend_yield * T) * norm.pdf(d1) * np.sqrt(T) / 100
    ...
    def calculate_theta_pnl(self, K, T, sigma, days=1):
        ...
        call_theta = (-self.spot_price * np.exp(-self.dividend_yield * T) * norm.pdf(d1) * sigma / (2 * np.sqrt(T))
                      + self.dividend_yield * self.spot_price *
                      np.exp(-self.dividend_yield * T) * norm.cdf(d1)
    ...
    def plot_rehedge_requirements(self, K, T, sigma, spot_range=0.10):
        ...
        ax1.plot(df['spot_price'], df['delta'], 'b-',
                 linewidth=2, label='Straddle Delta')
        ax1.axhline(y=0, color='black', linestyle='--', alpha=0.3)
        ax1.fill_between(df['spot_price'], -0.10, 0.10, alpha=0.2,
                         color='green', label='No Rehedge Zone (±0.10)')
        ax1.set_ylabel('Delta', fontsize=10)
        ax1.set_title('Straddle Delta vs Spot Price',
        ...
        ax2.axhline(y=0.10, color='red', linestyle='--',
                    alpha=0.5, label='Rehedge Threshold')
    ...
    def plot_hedge_pnl_breakdown(self, K, T, current_iv, forecast_iv):
        ...
        ax.fill_between(df['spot_move_pct'], 0,
                        df['gamma_pnl'], alpha=0.6, label='Gamma P&L')
        ...
        ax.plot(df['spot_move_pct'], df['total_pnl'],
                'k-', linewidth=2, label='Total P&L')
        ax.axhline(y=0, color='black', linestyle='--', alpha=0.3)

        ax.set_xlabel('Spot Move (%)', fontsize=10)
        ax.set_ylabel('P&L', fontsize=10)
        ax.set_title('Delta-Hedged Straddle P&L Breakdown (Gamma + Vega + Theta)',
                     fontsize=12, fontweight='bold')
        ax.legend(loc='best')
    ...
    def print_hedge_summary(self, K, T, sigma, forecast_iv=None):
        ...
        print(
            f"  Delta Neutral:           {'✓ YES' if hedge['is_delta_neutral'] else '✗ NO'}")

        if forecast_iv:
            vega_analysis = self.calculate_vega_pnl(K, T, sigma, forecast_iv)
            print(f"\nVega Exposure (if IV → {forecast_iv:.2%}):")
            print(
                f"  Current Value:           ${vega_analysis['current_value']:.4f}")
            print(
                f"  Forecast Value:          ${vega_analysis['forecast_value']:.4f}")
            print(
                f"  Vega P&L:                ${vega_analysis['vega_pnl']:+.4f} ({vega_analysis['vega_pnl_pct']:+.1f}%)")
