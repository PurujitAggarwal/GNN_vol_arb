...
def should_enter(market_iv, garch_forecast, iv_history, regime_blocker, earnings_blocker, current_date):
    ...
    if not garch_signal:
        entry_notes.append(
            f"Market IV not expensive (diff {iv_diff:.1%} < 3%)")
    if not iv_expensive:
...
def rolling_window_backtest(ticker, train_window=126, refit_frequency=21,
    ...
    if forecaster_method == 'gnn':
        ...
        forecaster = get_forecaster('egarch', stock_data=stock_data,
                                    train_window=train_window,
                                    refit_frequency=refit_frequency, verbose=verbose)
    ...
    for i, current_date in enumerate(trading_dates):
        ...
        try:
            ...
            forecaster.record_market_iv(market_iv)
    ...
    if forecaster_method == 'egarch':
        output_diagnostics(forecaster._forecaster)
    else:
        print(f"\n  Forecaster: GNN-HAR (horizon=5)")
...
def plot_short_vol_results(results_df, save_path='results/graphs/backtest_short_vol_analysis.png'):
    ...
    if len(trade_log) > 0 and 'net_pnl' in trade_log.columns:
        ...
        ax3.axis('off')
        ax3.text(0.5, 0.5, 'No completed trades to plot',
                 ha='center', va='center')
    ...
    if 'forecast_iv' in results_df.columns and 'market_iv' in results_df.columns:
        ...
        ax5.plot(dates, forecast_iv, label='Forecast (EGARCH + RP)',
                 linewidth=2, color='#2E86AB', alpha=0.85)
    ...
    ) * 100) if total_trades > 0 and 'net_pnl' in trade_log.columns else 0.0
    total_net_pnl = float(trade_log['net_pnl'].sum(
    )) if total_trades > 0 and 'net_pnl' in trade_log.columns else 0.0
...
if __name__ == "__main__":
    ...
    if results is not None:
        tag = 'GNN_VOL' if args.forecaster == 'gnn' else 'SHORT_VOL'
        results.to_csv(
            f"results/res/backtest_results_{ticker.lower()}_{tag}.csv", index=False)
        print(
            f"\nDaily results saved to results/res/backtest_results_{ticker.lower()}_{tag}.csv")

        trade_log = results.attrs.get('trade_log', pd.DataFrame())
        if len(trade_log) > 0:
            trade_log.to_csv(
                f"results/trade_log/trade_log_{ticker.lower()}_{tag}.csv", index=False)
            print(
                f"Trade log saved to results/trade_log/trade_log_{ticker.lower()}_{tag}.csv")

        plot_short_vol_results(
            results, save_path=f"results/graphs/backtest_short_vol_{ticker.lower()}_{tag}_analysis.png")
