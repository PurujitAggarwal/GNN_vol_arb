...
def get_atm_option_for_dte(options_df, date, spot_price, target_dte=45, dte_range=(30, 45), options_by_date=None):
    ...
    if len(calls) == 0 or len(puts) == 0:
        return None
    return {
        'strike': atm_strike, 'dte': target_dte_actual, 'maturity': target_dte_actual / 365,
        'call_price': calls['market_price'].iloc[0], 'put_price': puts['market_price'].iloc[0],
        'straddle_price': calls['market_price'].iloc[0] + puts['market_price'].iloc[0],
...
def should_enter(market_iv, garch_forecast, iv_history, regime_blocker, earnings_blocker, current_date):
    ...
    garch_signal = iv_diff > 0.05

    # Never sell vol when market IV is already extreme — regime is too dangerous
    if market_iv > 0.80:
        return 'HOLD', iv_diff, 50, False, [f"Market IV too high ({market_iv:.0%} > 80%)"]
    ...
    if not garch_signal:
        entry_notes.append(
            f"Market IV not expensive (diff {iv_diff:.1%} < 5%)")
    if not iv_expensive:
    ...
    signal = 'SELL' if (
        garch_signal and iv_expensive and not is_blocked) else 'HOLD'
...
def build_result_row(current_date, spot_price, garch_forecast, portfolio_value,
    ...
    return {
        'date': current_date, 'spot_price': spot_price, 'forecast_iv': garch_forecast,
        'market_iv': market_iv, 'iv_spread': iv_spread, 'iv_percentile': iv_percentile,
        'signal': signal, 'blocked': blocked, 'entry_notes': entry_notes,
        'atm_strike': atm_strike, 'atm_dte': atm_dte, 'straddle_price': straddle_price,
        'position_status': position_status, 'days_held': days_held,
        'iv_change_pct': iv_change_pct, 'portfolio_value': portfolio_value,
        'trade_pnl_dollars': trade_pnl_dollars, 'traded': traded,
    }
...
def output_diagnostics(volForecaster: VolForecaster):
    ...
    if diagnostics:
        ...
        print(
            f"    Mean Forecast:          {diagnostics['mean_forecast']:.2%}")
        print(
            f"    Mean Market IV:         {diagnostics['mean_market_iv']:.2%}")
        ...
        print(
            f"    Vol Risk Premium:       {diagnostics['vol_risk_premium']:.2%}")
...
def output_performance_analysis(df_trades):
    if len(df_trades) > 0:
        ...
        print(
            f"    Option P&L:         ${df_trades['option_pnl'].sum():>10,.2f}  (profit when IV drops)")
        print(
            f"    Gamma P&L:          ${df_trades['gamma_pnl'].sum():>10,.2f}  (cost from spot moves)")
        print(
            f"    Theta P&L:          ${df_trades['theta_pnl'].sum():>10,.2f}  (GAIN from decay)")
        print(
...
def get_trading_dates(options_data, train_window, start_date=None, end_date=None):
    ...
    if start_date is not None:
        trading_dates = [d for d in trading_dates if d >=
    ...
    print(
        f"Training window: {train_window} days, will start backtesting from day {train_window + 1}")
...
def get_regime_blocker(trading_dates, train_window, stock_data):
    ...
    print(
        f"Training regime blocker on data up to {backtest_start_date.date()} ({len(log_returns_historical)} days)")
    try:
        regime_blocker = RegimeBlockerXGB(
            returns=log_returns_historical, stress_vol_percentile=90.0,
            stress_drawdown_threshold=-0.05, calm_vol_percentile=25.0,
            block_threshold=0.5, random_state=42, verbose=False)
...
def rolling_window_backtest(ticker, train_window=126, refit_frequency=21,
                            ...
                            earnings_before=7, earnings_after=2, earnings_exit=3,
                            verbose=False):
    ...
    trading_dates = get_trading_dates(
        options_data, train_window, start_date, end_date)
    if not trading_dates:
        return None

    earnings_blocker = None
    if use_earnings_blocker:
        earnings_blocker = EarningsBlocker(ticker=ticker, earnings_csv=earnings_csv,
                                           before_days=earnings_before, after_days=earnings_after,
                                           exit_days=earnings_exit, verbose=verbose)
    regime_blocker = None
    if use_regime_blocker:
        regime_blocker = get_regime_blocker(
            trading_dates, train_window, stock_data)
    ...
    volForecaster = VolForecaster(
        stock_data, ticker=ticker, model=MODEL, verbose=verbose)
    ...
    for i, current_date in enumerate(trading_dates):
        if i % 20 == 0:
            ...
            print(f"\r  Progress: {i}/{total_dates} dates ({progress_pct:.1f}%) - "
                  f"Trades: {trades_entered} entered, {trades_exited} exited, "
                  f"Blocked: {blocked_count}", end="", flush=True)
        ...
        if active_position is not None:
            updated = active_position.update(
                current_date, spot_price, options_by_date)
            if not updated:
                # If position stuck for 20+ calendar days, abandon it
                days_since_entry = (
                    current_date - active_position.entry_date).days
                if days_since_entry > 20:
                    active_position = None
                continue

            should_exit, exit_reason = active_position.check_exit(
                current_date, earnings_blocker)
            trade_record = None
            if should_exit:
                trade_record = active_position.close(
                    current_date, spot_price, tc_calc, ticker_upper)
        ...
        try:
            iv_calc = ImpliedVolSurface(spot_price=spot_price, risk_free_rate=0.02,
                                        dividend_yield=dividend_yield, verbose=False)
            ...
            if signal == 'SELL':
                # FIX: Position.open() does NOT take iv_percentile — match the repo signature
                ...
                if active_position is None:
                    ...
                    if verbose:
                        print(f"\n[{current_date.date()}] ENTRY (SELL): Strike ${atm_option['strike']:.0f}, "
                              f"DTE {atm_option['dte']}, IV {market_iv:.1%}, Forecast {garch_forecast:.1%}, "
                              f"Credit ${active_position.entry_credit:.0f}")

            results.append(build_result_row(
                current_date, spot_price, garch_forecast, portfolio_value,
                market_iv=market_iv, iv_spread=iv_diff,
                iv_percentile=iv_percentile if len(
                    iv_history) >= 20 else np.nan,
                signal=signal, blocked=is_blocked,
                entry_notes='; '.join(entry_notes) if entry_notes else '',
                atm_strike=atm_option['strike'], atm_dte=atm_option['dte'],
                straddle_price=atm_option['straddle_price'], traded=signal == 'SELL',
            ))
            processed_count += 1
...
def plot_short_vol_results(results_df, save_path=None):
    ...
    ax1.set_title('Portfolio Value Over Time (Short Vol Strategy)',
    ...
    if len(trade_log) > 0 and 'net_pnl' in trade_log.columns:
        ax3.hist(trade_log['net_pnl'].astype(float).values,
                 bins=24, alpha=0.75, edgecolor='black', color='#E63946')
        ax3.axvline(x=0, color='black', linestyle='--',
                    linewidth=2, label='Break-even')
        ax3.set_title('Trade Net P&L Distribution',
    ...
    if len(trade_log) > 0 and 'exit_reason' in trade_log.columns:
        ...
        ax4.barh(reason_counts.index[::-1], reason_counts.values[::-1],
    ...
    if 'forecast_iv' in results_df.columns and 'market_iv' in results_df.columns:
        ...
        ax5.plot(dates, forecast_iv,
                 label=f'Forecast ({MODEL})', linewidth=2, color='#2E86AB', alpha=0.85)
        ax5.plot(dates, market_iv, label='Market IV',
                 linewidth=2, color='#F18F01', alpha=0.85)
        ...
        ax5.set_title('Forecast vs Market IV (Signals Highlighted)',
                      fontsize=12, fontweight='bold')
    ...
    total_trades = len(trade_log) if len(trade_log) > 0 else 0
    win_rate = float((trade_log['net_pnl'] > 0).mean()
                     * 100) if total_trades > 0 else 0
    total_net_pnl = float(
        trade_log['net_pnl'].sum()) if total_trades > 0 else 0
    ...
    plt.suptitle('Short Vol Strategy - Performance Dashboard',
...
if __name__ == "__main__":
    ...
    results = rolling_window_backtest(
        ticker=ticker, train_window=126, refit_frequency=21,
        starting_capital=100000, position_size=8000,
        use_regime_blocker=True, start_date=None, end_date=None,
        verbose=True, earnings_csv=f'data/{ticker.lower()}_earnings_dates.csv')

    if results is not None:
        os.makedirs(f"{RESULTS_DIR}/res", exist_ok=True)
        results.to_csv(
            f"{RESULTS_DIR}/res/backtest_results_{ticker.lower()}_SHORT_VOL.csv", index=False)
        print(
        ...
        if len(trade_log) > 0:
            ...
            trade_log.to_csv(
                f"{RESULTS_DIR}/trade_log/trade_log_{ticker.lower()}_SHORT_VOL.csv", index=False)
            print(
                f"Trade log saved to {RESULTS_DIR}/trade_log/trade_log_{ticker.lower()}_SHORT_VOL.csv")

        plot_short_vol_results(
            results, save_path=f"{RESULTS_DIR}/graphs/backtest_short_vol_{ticker.lower()}_analysis.png")
