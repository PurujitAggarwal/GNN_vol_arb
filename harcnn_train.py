    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            if no_improve >= PATIENCE:
                print(
                    f"Early stop epoch {epoch} | best_val_loss {best_val_loss:.4f}")
                break

        if epoch % 50 == 0:
            print(
                f"Epoch {epoch:4d} | train {train_loss:.4f} | val {val_loss:.4f} | acc {val_acc:.4f}")
...
def train_harcnn():
    ...
    train_loader = DataLoader(
        RVDataset(x_train, y_train), batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(RVDataset(x_val,   y_val),
