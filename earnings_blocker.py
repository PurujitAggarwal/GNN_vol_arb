    ...
    def _load_earnings(self, filepath):
        ...
        try:
            ...
            if len(ticker_data) == 0:
                if self.verbose:
                    print(
                        f"  [EarningsBlocker] No earnings data for {self.ticker}")
                return np.array([])
            ...
            if self.verbose:
                print(
                    f"  [EarningsBlocker] Loaded {len(earnings_dates)} earnings dates for {self.ticker}")
    ...
    def should_block_entry(self, current_date):
        ...
        if prev_earn is not None:
            ...
            if 0 <= days_since <= self.after_days:
                ...
                if self.verbose:
                    print(
                        f"  [EarningsBlocker] Entry blocked: {self.last_block_reason}")
                return True
    ...
    def should_force_exit(self, current_date):
        ...
        if next_earn is not None:
            ...
            if 0 < days_until <= self.exit_days:
                ...
                if self.verbose:
                    print(
                        f"  [EarningsBlocker] Forced exit: {self.last_block_reason}")
                return True
