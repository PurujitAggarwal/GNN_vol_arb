...
def load_options_data(filepath, ticker=None):
    ...
    if ticker:
        ...
        if len(df) == 0:
            ...
            print(
                f"  Available tickers: {sorted(df['ticker'].unique())[:10]}... ({len(df['ticker'].unique())} total)")
...
def get_iv_surface_for_date(options_df, date, spot_price, risk_free_rate=0.02, dividend_yield=0.02):
    ...
    for i, K in enumerate(strikes):
        for j, T in enumerate(maturities):
            price_data = calls[(calls['strike_price'] == K)
                               & (calls['maturity'] == T)]
            if len(price_data) > 0:
...
def get_atm_option_for_dte(options_df, date, spot_price, target_dte=45, dte_range=(30, 45)):
    ...
    target_dte_actual = date_options.loc[date_options['dte_diff'].idxmin(
    ), 'days_to_expiry']

    target_options = date_options[date_options['days_to_expiry']
                                  == target_dte_actual].copy()
    ...
    calls = target_options[(target_options['strike_price'] == atm_strike) & (
        target_options['cp_flag'] == 'C')]
    puts = target_options[(target_options['strike_price'] == atm_strike) & (
        target_options['cp_flag'] == 'P')]
...
def get_iv_for_option(iv_calc, strike, maturity, call_price, put_price):
    try:
        call_iv = iv_calc.implied_volatility(
            call_price, strike, maturity, 'call')
...
def rolling_window_backtest(ticker, train_window=252, refit_frequency=21,
    ...
    print(
        f"  [i] Training window: {train_window} days, will start backtesting from day {train_window + 1}")

    if len(trading_dates) <= train_window:
        print(
            f"  [!] Error: Not enough data! Need >{train_window} days, have {len(trading_dates)}")
        return None
    ...
    if use_regime_blocker:
        ...
        # Exclude backtest start date
        historical_stock_data = stock_data.loc[:backtest_start_date].iloc[:-1]
        log_returns_historical = parse_data(
            historical_stock_data.reset_index(), price_col='prc')

        print(
            f"  [i] Training regime blocker on data up to {backtest_start_date.date()} ({len(log_returns_historical)} days)")
        print(
            f"  [i] This ensures no look-ahead bias - blocker never sees backtest period data")
        ...
        try:
            ...
            print(
                "  * Regime blocker initialized successfully (trained on pre-backtest data only)")
    ...
    print(f"  Forecaster: {forecaster_method.upper()}")
    last_garch_fit_date = None
    garch_forecast = None
    if forecaster_method == 'gnn':
        _forecaster = get_forecaster(
            'gnn', ticker=ticker, horizon=5, verbose=verbose)
    ...
    for i, current_date in enumerate(trading_dates):
        ...
        if forecaster_method == 'gnn':
            garch_forecast = _forecaster.forecast(current_date)
            if garch_forecast is None:
                continue
        else:
            days_since_fit = (
                current_date - last_garch_fit_date).days if last_garch_fit_date else refit_frequency + 1
            ...
            if days_since_fit >= refit_frequency or garch_forecast is None:
                ...
                historical_data = stock_data.loc[:
                                                 current_date].iloc[-train_window-1:-1]
                log_returns = parse_data(historical_data, price_col='prc')

                try:
                    _, sigma_forecast_base = garch_modelling(
                        log_returns, DE_MEAN, MODEL, DISTRIBUTION, validity_checks=False)

                    sigma_forecast_ensemble = ensemble_vol_forecast(
                        log_returns, sigma_forecast_base)

                    vol_risk_premium = calculate_vol_risk_premium(
                        forecast_history, market_iv_history)
                    ...
                    if verbose:
                        ...
                        print(
                            f"    Base EGARCH:     {sigma_forecast_base:.2%}")
                        print(
                            f"    Ensemble:        {sigma_forecast_ensemble:.2%}")
        ...
        if active_position is not None:
            ...
            current_option = get_atm_option_for_dte(
                ...
                dte_range=(max(
                    1, active_position['dte_remaining'] - 5), active_position['dte_remaining'] + 5)
            )
            ...
            if len(strike_options) > 0:
                ...
                if len(calls) > 0 and len(puts) > 0:
                    ...
                    calls = calls.iloc[(
                        calls['exdate'] - active_position['exdate']).abs().argsort()[:1]]
                    puts = puts.iloc[(
                        puts['exdate'] - active_position['exdate']).abs().argsort()[:1]]

                    if len(calls) > 0 and len(puts) > 0:
                        current_call_price = calls['market_price'].iloc[0]
                        current_put_price = puts['market_price'].iloc[0]
                        current_straddle_value = (
                            current_call_price + current_put_price) * 100 * active_position['num_straddles']

                        # Calculate current IV
                        current_dte = max(
                            1, active_position['entry_dte'] - days_held)
            ...
            if current_straddle_value is None or current_iv is None:
                ...
                active_position['dte_remaining'] = max(
                    1, active_position['entry_dte'] - days_held)
                continue
            ...
            hedger = DeltaHedger(
                spot_price, risk_free_rate=0.02, dividend_yield=0.02)
            current_maturity = max(
                0.01, (active_position['entry_dte'] - days_held) / 365)
            ...
            prev_spot = active_position.get(
                'prev_spot', active_position['entry_spot'])
            ...
            delta_hedge_pnl = -prev_delta * spot_change * \
                active_position['num_straddles']

            gamma_pnl = 0.5 * prev_gamma * \
                (spot_change ** 2) * active_position['num_straddles']
            ...
            if 'prev_hedge_units' in active_position:
                hedge_adjustment = abs(
                    hedge_result['hedge_units'] - active_position['prev_hedge_units'])
                hedge_rebalance_cost = hedge_adjustment * \
                    spot_price * 0.001  # 0.1% transaction cost
            else:
            ...
            active_position['cumulative_hedge_pnl'] = active_position.get(
                'cumulative_hedge_pnl', 0) + daily_hedge_pnl
            active_position['cumulative_gamma_pnl'] = active_position.get(
                'cumulative_gamma_pnl', 0) + gamma_pnl
            active_position['cumulative_theta_pnl'] = active_position.get(
                'cumulative_theta_pnl', 0) + theta_pnl
            active_position['cumulative_delta_pnl'] = active_position.get(
                'cumulative_delta_pnl', 0) + delta_hedge_pnl
            active_position['total_hedge_cost'] = active_position.get(
                'total_hedge_cost', 0) + hedge_rebalance_cost
            ...
            # Use calculated straddle delta
            active_position['prev_delta'] = straddle_delta
            # Use calculated straddle gamma
            ...
            if should_exit:
                ...
                option_pnl = current_straddle_value - \
                    active_position['entry_cost']

                # 2. Cumulative hedge P&L (gamma + theta + delta hedge)
                cumulative_hedge_pnl = active_position.get(
                    'cumulative_hedge_pnl', 0)

                # 3. Exit transaction costs
                exit_cost = tc_calc.calculate(
                    price=current_straddle_value /
                    (100 * active_position['num_straddles']),
                ...
                total_hedge_rebalance_costs = active_position.get(
                    'total_hedge_cost', 0)
        ...
        if strikes is None:
            ...
            if verbose:
                print(
                    f"  [{current_date.date()}] Insufficient options data, skipping...")
            continue
        ...
        try:
            ...
            if atm_option is None:
                ...
                if verbose:
                    print(
                        f"  [{current_date.date()}] No 30-45 DTE options available")
                continue
            ...
            if len(iv_history) >= 20:  # Need some history
                iv_percentile = (np.array(iv_history) <
                                 market_iv).sum() / len(iv_history) * 100
            ...
            if regime_blocker is not None:
                try:
                    is_blocked = regime_blocker.isBlocked(
            ...
            if signal == 'BUY':
                # Per straddle (100 shares)
                straddle_price_per_unit = atm_option['straddle_price'] * 100
                num_straddles = max(
                    1, int(position_size / straddle_price_per_unit))
                ...
                straddle_gamma = np.exp(-0.02 * atm_option['maturity']) * norm.pdf(d1) / \
                    (spot_price * market_iv *
                     np.sqrt(atm_option['maturity'])) * 2  # x2 for straddle
    ...
    print(
        f"  Processed: {processed_count}, Skipped: {skipped_count}, Blocked: {blocked_count}")
    print(
        f"  Trades entered: {trades_entered}, Trades exited: {trades_exited}")
    ...
    if len(forecast_history) > 0 and len(market_iv_history) > 0:
        ...
        print(
            f"    RMSE:                   {np.sqrt((forecast_error**2).mean()):.2%}")
        print(
            f"    Correlation:            {np.corrcoef(forecast_arr, market_arr)[0,1]:.3f}")
        print(
            f"    Calibrated Risk Prem:   {calculate_vol_risk_premium(forecast_history, market_iv_history):.2%}")
    ...
    print(
        f"  * Regime blocker prevented {blocked_count} entries during stress periods")
    ...
    if len(df_trades) > 0:
        ...
        print(
            f"    Win Rate:         {(df_trades['net_pnl'] > 0).mean()*100:>6.1f}%")

        print(f"\n  Holding Period:")
        print(
            f"    Mean:             {df_trades['days_held'].mean():>6.1f} days")
        print(
            f"    Median:           {df_trades['days_held'].median():>6.1f} days")
        print(f"    Min:              {df_trades['days_held'].min():>6} days")
        print(f"    Max:              {df_trades['days_held'].max():>6} days")

        print(f"\n  IV Change (Entry to Exit):")
        print(
            f"    Mean:             {df_trades['iv_change_pct'].mean():>7.2f}%")
        print(
            f"    Median:           {df_trades['iv_change_pct'].median():>7.2f}%")
        print(
            f"    Std:              {df_trades['iv_change_pct'].std():>7.2f}%")
        ...
        print(
            f"    Median Trade P&L: ${df_trades['net_pnl'].median():>10,.2f}")
        ...
        print(f"\n  P&L Breakdown:")
        print(
            f"    Option P&L:         ${df_trades['option_pnl'].sum():>10,.2f}")
        print(
            f"    Gamma P&L:          ${df_trades['gamma_pnl'].sum():>10,.2f}")
        print(
            f"    Theta P&L:          ${df_trades['theta_pnl'].sum():>10,.2f}")
        print(
            f"    Delta Hedge P&L:    ${df_trades['delta_hedge_pnl'].sum():>10,.2f}")
        print(
            f"    Total Hedge P&L:    ${df_trades['hedge_pnl'].sum():>10,.2f}")

        print(f"\n  Transaction Costs:")
        print(
            f"    Entry Costs:        ${df_trades['entry_tc'].sum():>10,.2f}")
        print(
            f"    Exit Costs:         ${df_trades['exit_cost'].sum():>10,.2f}")
        print(
            f"    Hedge Rebal Costs:  ${df_trades['hedge_rebalance_costs'].sum():>10,.2f}")
    ...
    if 'iv_spread' in df_results.columns:
        ...
        if len(iv_spreads) > 0:
            ...
            print(
                f"    % Above 4%:       {(iv_spreads > 0.04).mean()*100:>6.1f}%")

    print(f"\n  Final Portfolio Value: ${portfolio_value:,.2f}")
    print(
        f"  Total Return:          {(portfolio_value - starting_capital) / starting_capital * 100:.2f}%")
...
def calculate_performance_metrics(results_df, starting_capital=100000, annual_risk_free_rate=0.02):
    ...
    metrics['total_return'] = (
        portfolio_values[-1] - starting_capital) / starting_capital

    metrics['total_trades'] = len(trade_log)
    metrics['total_observations'] = len(results_df)
    metrics['mean_return'] = portfolio_returns.mean() if len(
        portfolio_returns) > 0 else 0
    metrics['median_return'] = portfolio_returns.median() if len(
        portfolio_returns) > 0 else 0
    metrics['std_return'] = portfolio_returns.std() if len(
        portfolio_returns) > 0 else 0
    metrics['min_return'] = portfolio_returns.min() if len(
        portfolio_returns) > 0 else 0
    metrics['max_return'] = portfolio_returns.max() if len(
        portfolio_returns) > 0 else 0
    ...
    metrics['total_transaction_costs'] = metrics['total_entry_costs'] + \
        metrics['total_exit_costs'] + metrics['total_hedge_rebalance_costs']
    metrics['transaction_cost_pct'] = (
        metrics['total_transaction_costs'] / starting_capital) * 100

    metrics['win_rate'] = (trade_pnl > 0).mean()
    metrics['num_wins'] = (trade_pnl > 0).sum()
    metrics['num_losses'] = (trade_pnl < 0).sum()

    metrics['exits_take_profit'] = trade_log['exit_reason'].str.contains(
        'profit', case=False).sum()
    metrics['exits_stop_loss'] = trade_log['exit_reason'].str.contains(
        'stop', case=False).sum()
    metrics['exits_mandatory'] = trade_log['exit_reason'].str.contains(
        'mandatory|Day 14', case=False).sum()
        ...
        if len(downside_returns) > 0:
            ...
            if downside_std > 0:
                ...
                metrics['sortino_ratio'] = (
                    portfolio_returns.mean() - daily_rf) / downside_std * np.sqrt(252)
            else:
...
def plot_backtest_results(results_df, save_path='backtest_analysis.png'):
    ...
    ax1.plot(dates, portfolio_values, linewidth=2,
             color='#2E86AB', label='Portfolio Value')
    ax1.fill_between(dates, starting_value, portfolio_values,
                     alpha=0.3, color='#2E86AB')
    ax1.axhline(y=starting_value, color='red', linestyle='--',
    ...
    if 'iv_spread' in results_df.columns:
        ...
        ax3.hist(iv_spreads, bins=30, color='#F18F01',
                 alpha=0.7, edgecolor='black')
        ax3.axvline(x=4, color='green', linestyle='--',
                    linewidth=2, label='Entry threshold (4%)')
        ax3.axvline(x=0, color='red', linestyle='--',
                    linewidth=2, label='Zero spread')
    ax3.set_title('GARCH - Market IV Spread Distribution',
    ...
    if len(trade_log) > 0:
        ax4.hist(trade_log['net_pnl'], bins=20,
    ...
    if len(trade_log) > 0:
        ax5.hist(trade_log['iv_change_pct'], bins=20,
                 color='#BC4B51', alpha=0.7, edgecolor='black')
        ax5.axvline(x=0, color='black', linestyle='-', linewidth=1)
        ax5.axvline(x=3, color='green', linestyle='--',
                    alpha=0.7, label='Day 7 profit (+3%)')
        ax5.axvline(x=-2, color='red', linestyle='--',
                    alpha=0.7, label='Day 7 stop (-2%)')
    ax5.set_title('IV Change During Trade (Entry to Exit)',
                  fontsize=12, fontweight='bold')
    ...
    if len(trade_log) > 0:
        ...
        ax6.hist(holding_days, bins=range(1, 16), color='#5C8A97',
                 alpha=0.7, edgecolor='black', align='left')
        ax6.axvline(x=7, color='orange', linestyle='--',
                    linewidth=2, alpha=0.7, label='Day 7 check')
        ax6.axvline(x=10, color='orange', linestyle=':',
                    linewidth=2, alpha=0.7, label='Day 10 check')
        ax6.axvline(x=14, color='red', linestyle='--',
                    linewidth=2, alpha=0.7, label='Day 14 exit')
    ...
    if len(trade_log) > 0:
        ...
        ax8.plot(trade_numbers, cumulative_pnl, linewidth=2,
                 color='#2E86AB', marker='o', markersize=4)
        ax8.axhline(y=0, color='red', linestyle='--', alpha=0.5)
        ax8.fill_between(trade_numbers, 0, cumulative_pnl,
                         alpha=0.3, color='#2E86AB')
...
def print_performance_summary(metrics):
    ...
    print("\n[Trading Statistics]")
    print(
        f"  Total Observations:  {metrics.get('total_observations', metrics['total_trades']):>10}")
    print(f"  Actual Trades:       {metrics['total_trades']:>10}")
    print(
        f"  Mean Holding Days:   {metrics.get('mean_holding_days', 0):>10.1f}")
    ...
    print(
        f"  Trades IV Up:        {metrics.get('trades_with_iv_increase', 0):>10}")
    print(
        f"  Trades IV Down:      {metrics.get('trades_with_iv_decrease', 0):>10}")

    print("\n[P&L Breakdown]")
    print(
        f"  Option P&L:          ${metrics.get('total_option_pnl', 0):>10,.2f}")
    print(
        f"  Gamma P&L:           ${metrics.get('total_gamma_pnl', 0):>10,.2f}")
    print(
        f"  Theta P&L:           ${metrics.get('total_theta_pnl', 0):>10,.2f}")
    print(
        f"  Delta Hedge P&L:     ${metrics.get('total_delta_hedge_pnl', 0):>10,.2f}")
    print(
        f"  Total Hedge P&L:     ${metrics.get('total_hedge_pnl', 0):>10,.2f}")

    print("\n[Transaction Costs]")
    print(
        f"  Entry Costs:         ${metrics.get('total_entry_costs', 0):>10,.2f}")
    print(
        f"  Exit Costs:          ${metrics.get('total_exit_costs', 0):>10,.2f}")
    print(
        f"  Hedge Rebal Costs:   ${metrics.get('total_hedge_rebalance_costs', 0):>10,.2f}")
    print(
        f"  Total Costs:         ${metrics['total_transaction_costs']:>10,.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--forecaster', default='egarch',
                        choices=['egarch', 'gnn'])
    args = parser.parse_args()
    ticker = args.ticker
    print(f"Ticker: {ticker} | Forecaster: {args.forecaster}")

    results = rolling_window_backtest(
        ticker=ticker,
        forecaster_method=args.forecaster,
        train_window=126,
        refit_frequency=21,
    ...
    if results is not None:
        tag = 'GNN_VOL' if args.forecaster == 'gnn' else 'LONG_VOL'
        results.to_csv(
            f"backtest_results_{ticker.lower()}_{tag}.csv", index=False)
        print(
            f"\nDaily results saved to backtest_results_{ticker.lower()}_{tag}.csv")

        trade_log = results.attrs.get('trade_log', pd.DataFrame())
        if len(trade_log) > 0:
            trade_log.to_csv(
                f"trade_log_{ticker.lower()}_{tag}.csv", index=False)
            print(f"Trade log saved to trade_log_{ticker.lower()}_{tag}.csv")

        print("\nCalculating performance metrics...")
        metrics = calculate_performance_metrics(
            results, starting_capital=100000)
        print_performance_summary(metrics)

        print("\nGenerating performance charts...")
        plot_backtest_results(
            results, save_path=f'backtest_analysis_{ticker.lower()}_{tag}.png')
