...
def combined_vol_arbitrage_backtest(ticker, train_window=126, refit_frequency=21,
    ...
    def manage_position(position, position_type, current_date, spot_price, options_data,
        ...
        if len(strike_options) > 0:
            ...
            if len(calls) > 0 and len(puts) > 0:
                calls = calls.iloc[(
                    calls['exdate'] - position['exdate']).abs().argsort()[:1]]
                puts = puts.iloc[(
                    puts['exdate'] - position['exdate']).abs().argsort()[:1]]

                if len(calls) > 0 and len(puts) > 0:
                    current_call_price = calls['market_price'].iloc[0]
                    current_put_price = puts['market_price'].iloc[0]
                    current_straddle_value = (
                        current_call_price + current_put_price) * 100 * position['num_straddles']
        ...
        if current_straddle_value is None or current_iv is None:
            position['dte_remaining'] = max(
                1, position['entry_dte'] - days_held)
        ...
        gamma_pnl = 0.5 * prev_gamma * \
            (spot_change ** 2) * position['num_straddles']
        ...
        if 'prev_hedge_units' in position:
            hedge_adjustment = abs(
                hedge_result['hedge_units'] - position['prev_hedge_units'])
        ...
        # Accumulate
        position['cumulative_hedge_pnl'] = position.get(
            'cumulative_hedge_pnl', 0) + daily_hedge_pnl
        position['cumulative_gamma_pnl'] = position.get(
            'cumulative_gamma_pnl', 0) + gamma_pnl
        position['cumulative_theta_pnl'] = position.get(
            'cumulative_theta_pnl', 0) + theta_pnl
        position['cumulative_delta_pnl'] = position.get(
            'cumulative_delta_pnl', 0) + delta_hedge_pnl
        position['total_hedge_cost'] = position.get(
    ...
    for i, current_date in enumerate(trading_dates):
        ...
        try:
            ...
            if long_signal and long_position is None and not is_blocked and iv_cheap:
                ...
                long_size = calculate_dynamic_position_size(
                    long_size, market_iv, iv_history, 'LONG')

                if long_size > 0:
                    straddle_price_per_unit = atm_option['straddle_price'] * 100
                    num_straddles = max(
                        1, int(long_size / straddle_price_per_unit))
            ...
            if short_signal and short_position is None and not is_blocked:
                ...
                short_size = calculate_dynamic_position_size(
                    short_size, market_iv, iv_history, 'SHORT')

                if short_size > 0:
                    straddle_price_per_unit = atm_option['straddle_price'] * 100
                    num_straddles = max(
                        1, int(short_size / straddle_price_per_unit))
                    entry_cost = straddle_price_per_unit * num_straddles
    ...
    df_long_trades = pd.DataFrame(
        long_trades) if long_trades else pd.DataFrame()
    df_short_trades = pd.DataFrame(
        short_trades) if short_trades else pd.DataFrame()
    ...
    if len(df_all_trades) > 0:
        ...
        print(
            f"    Win Rate:             {(df_all_trades['net_pnl'] > 0).mean()*100:.1f}%")
        print(
            f"    Total Net P&L:        ${df_all_trades['net_pnl'].sum():,.2f}")
        print(
            f"    Mean Trade P&L:       ${df_all_trades['net_pnl'].mean():,.2f}")
        print(
            f"    Best Trade:           ${df_all_trades['net_pnl'].max():,.2f}")
        print(
            f"    Worst Trade:          ${df_all_trades['net_pnl'].min():,.2f}")

        if len(df_long_trades) > 0:
            print(f"\n  === LONG VOL COMPONENT ===")
            print(f"    Trades:               {len(df_long_trades)}")
            print(
                f"    Win Rate:             {(df_long_trades['net_pnl'] > 0).mean()*100:.1f}%")
            print(
                f"    Total P&L:            ${df_long_trades['net_pnl'].sum():,.2f}")
            print(
                f"    Avg Trade:            ${df_long_trades['net_pnl'].mean():,.2f}")
            print(
                f"    Option P&L:           ${df_long_trades['option_pnl'].sum():,.2f}")
            print(
                f"    Theta P&L:            ${df_long_trades['theta_pnl'].sum():,.2f}")

        if len(df_short_trades) > 0:
            print(f"\n  === SHORT VOL COMPONENT ===")
            print(f"    Trades:               {len(df_short_trades)}")
            print(
                f"    Win Rate:             {(df_short_trades['net_pnl'] > 0).mean()*100:.1f}%")
            print(
                f"    Total P&L:            ${df_short_trades['net_pnl'].sum():,.2f}")
            print(
                f"    Avg Trade:            ${df_short_trades['net_pnl'].mean():,.2f}")
            print(
                f"    Option P&L:           ${df_short_trades['option_pnl'].sum():,.2f}")
            print(
                f"    Theta P&L:            ${df_short_trades['theta_pnl'].sum():,.2f}")
        ...
        if len(df_long_trades) > 5 and len(df_short_trades) > 5:
            ...
            if mask.sum() > 5:
                correlation = np.corrcoef(
                    long_daily_pnl[mask], short_daily_pnl[mask])[0, 1]
                ...
                if correlation < -0.2:
                    ...
                    print(
                        f"    ⚠ POSITIVE correlation - limited diversification benefit")

    print(f"\n  Final Portfolio Value: ${portfolio_value:,.2f}")
    print(
        f"  Total Return:          {(portfolio_value - starting_capital) / starting_capital * 100:.2f}%")
    print(
        f"  Annualized Return:     {((portfolio_value / starting_capital) ** (1/4.5) - 1) * 100:.2f}%")
        ...
        if len(returns) > 0:
            ...
            if len(downside_returns) > 0:
                ...
                sortino_ratio = (annual_return - risk_free_rate) / \
                    downside_vol if downside_vol > 0 else 0
            else:
...
def plot_combined_results(results_df, save_path='backtest_combined_analysis.png'):
    ...
    if len(long_trades) > 0:
        ...
        for entry_date in long_entries:
            if entry_date in dates.values:
                ...
                ax1.scatter(
                    entry_date, portfolio_values[idx], color='green', s=50, alpha=0.6, marker='^')
    ...
    if len(short_trades) > 0:
        ...
        for entry_date in short_entries:
            if entry_date in dates.values:
                idx = dates[dates == entry_date].index[0]
                ax1.scatter(
                    entry_date, portfolio_values[idx], color='red', s=50, alpha=0.6, marker='v')

    ax1.set_title('Portfolio Value Over Time (Combined Long/Short Strategy)',
    ...
    if len(all_trades) > 0:
        long_pnls = long_trades['net_pnl'].values if len(
            long_trades) > 0 else []
        short_pnls = short_trades['net_pnl'].values if len(
            short_trades) > 0 else []
        ...
        ax3.set_title('Trade P&L Distribution by Strategy',
    ...
    if len(long_trades) > 0 and len(short_trades) > 0:
        ...
        ax5.set_title('Cumulative P&L by Strategy',
                      fontsize=12, fontweight='bold')
    ...
    if len(all_trades) > 0:
        ...
        all_trades_df['entry_date'] = pd.to_datetime(
            all_trades_df['entry_date'])
        all_trades_df['year_month'] = all_trades_df['entry_date'].dt.to_period(
            'M')

        trade_counts = all_trades_df.groupby(
            ['year_month', 'position_type']).size().unstack(fill_value=0)

        trade_counts.plot(kind='bar', stacked=True, ax=ax6, color=['#6A994E', '#E63946'],
                          alpha=0.7, edgecolor='black')
        ax6.set_title('Trade Count by Month and Type',
                      fontsize=12, fontweight='bold')
    ...
    if len(long_trades) > 0 and len(short_trades) > 0:
        ...
        ax7.set_title('IV Change Distribution by Strategy',
                      fontsize=12, fontweight='bold')
    ...
    if len(all_trades) > 0:
        long_days = long_trades['days_held'].values if len(
            long_trades) > 0 else []
        short_days = short_trades['days_held'].values if len(
            short_trades) > 0 else []
        ...
        ax8.axvline(x=7, color='orange', linestyle='--',
                    linewidth=2, alpha=0.7, label='Day 7 check')
        ax8.axvline(x=10, color='orange', linestyle=':',
                    linewidth=2, alpha=0.7, label='Day 10 check')
        ax8.axvline(x=14, color='red', linestyle='--',
                    linewidth=2, alpha=0.7, label='Day 14 exit')
        ax8.set_title('Holding Period Distribution',
                      fontsize=12, fontweight='bold')
    ...
    if 'forecast_iv' in results_df.columns and 'market_iv' in results_df.columns:
        ...
        ax9.set_title('Forecast vs Market IV with Signals',
                      fontsize=12, fontweight='bold')
        ax9.set_ylabel('Implied Volatility (%)', fontsize=10)
        ax9.set_ylim(
            0, max(market_iv.max(), results_df['forecast_iv'].max() * 100) * 1.1)
        ax9.legend()
...
if __name__ == "__main__":
    ...
    if results is not None:
        tag = 'GNN_VOL' if args.forecaster == 'gnn' else 'COMBINED'
        results.to_csv(
            f"backtest_combined_{ticker.lower()}_{tag}.csv", index=False)
        print(
            f"\nResults saved to backtest_combined_{ticker.lower()}_{tag}.csv")
        ...
        if len(long_trades) > 0:
            long_trades.to_csv(
                f"trades_long_{ticker.lower()}_{tag}.csv", index=False)
            print(
                f"Long trades saved to trades_long_{ticker.lower()}_{tag}.csv")

        if len(short_trades) > 0:
            short_trades.to_csv(
                f"trades_short_{ticker.lower()}_{tag}.csv", index=False)
            print(
                f"Short trades saved to trades_short_{ticker.lower()}_{tag}.csv")

        if len(all_trades) > 0:
            all_trades.to_csv(
                f"trades_combined_{ticker.lower()}_{tag}.csv", index=False)
            print(
                f"Combined trades saved to trades_combined_{ticker.lower()}_{tag}.csv")

        print("\nGenerating performance charts...")
        plot_combined_results(
            results, save_path=f'backtest_combined_analysis_{ticker.lower()}_{tag}.png')
